...
const TakePracticeExam = () => {
  ...
  const handleEndSessionDueToFullscreenViolation = useCallback(async () => {
    try {
      ...
      await fetch(`${API_URLS.PRACTICE}/${sessionId}/end`, {
        method: 'POST',
        headers: {
      ...
      // Show violation message and redirect
      alert(`Practice session ended due to fullscreen violation. You exceeded the maximum allowed attempts (${maxFullscreenAttempts}) to exit fullscreen mode.`);
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      ...
      if (!currentlyFullscreen && fullscreenAttempts < maxFullscreenAttempts) {
        // User exited fullscreen - increment attempts and show warning
        setFullscreenAttempts(prev => prev + 1);
        setShowWarning(true);
        setWarningCountdown(10); // 10 seconds countdown
        ...
        // Max attempts reached - end session
  ...
  const loadQuestion = useCallback(async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URLS.PRACTICE}/${sessionId}/next`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (response.ok) {
        ...
        if (data.completed) {
          ...
          navigate(`/practice-exam/results/${sessionId}`);
          return;
        }
  ...
  }, [selectedAnswer, sessionId, navigate, questionNumber]);
    ...
    return (
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-white text-xl mb-2">Session Completed!</h2>
      ...
      {showWarning && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-red-100 border border-red-400 text-red-700 px-6 py-4 rounded-lg max-w-md mx-4">
            ...
            <p className="mb-4">
              You exited fullscreen mode. Practice exams must be taken in fullscreen to maintain exam integrity.
            </p>
            <div className="mb-4">
              <div className="bg-red-200 rounded p-3">
                ...
                <p className="text-xs mt-1">
                  {maxFullscreenAttempts - fullscreenAttempts} attempts remaining before session ends.
                </p>
              </div>
            </div>
...
export default TakePracticeExam;